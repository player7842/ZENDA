...
// ==================================================================
// GET /api/fichas/publicas — SIN auth, para el formulario de registro
// ==================================================================
// Solo fichas del programa ADSO (fijo por ahora, codigo_programa 2338).
// Expone lo mínimo: nada de conteos ni datos sensibles.
export const getFichasPublicas = async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT f.ficha_id, f.numero_ficha, f.jornada
      FROM fichas f
      JOIN programas p ON p.programa_id = f.programa_id
      WHERE p.codigo_programa = 2338
      ORDER BY f.numero_ficha
    `);
    res.json(result.rows);
  } catch (error) {
    console.error('Error al obtener fichas públicas:', error);
    res.status(500).json({ message: 'Error al obtener fichas' });
  }
};